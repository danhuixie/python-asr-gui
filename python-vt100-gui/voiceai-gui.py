...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        try:
            ...
            th1.start()
            th2 = threading.Thread(target = self.Backup)
            th2.start()
        except:
    ...
    def OnExit(self, e):
        # self.fp.write("1234567\n")
        # self.fp.close()
        # self.status_info.Clear()
        # self.status_info.AppendText(self.filename)
        # self.stream_asr.stop()
        self.Close(True)
        # e.Skip()
    ...
    def OnClose(self, e):
        ...
        time.sleep(0.5)
        if self.filename:
            txt_name = self.filename[0 : -4] + ".txt"
        else:
            txt_name = "test.txt"
        self.fp = open(txt_name, "w")
        # self.logger.AppendText("Online Result:1234567\n")
        text = self.logger.GetValue()
        self.fp.write(text)
        self.fp.close()
        # self.status_info.Clear()
        # self.status_info.AppendText(self.filename)
    ...
    def Backup(self):
        while self.RecordFlag:
            if self.filename:
                txt_name = self.filename[0 : -4] + "-back.txt"
            else:
                txt_name = "test-back.txt"
            self.fp = open(txt_name, "w")
            text = self.logger.GetValue()
            self.fp.write(text)
            self.fp.close()
